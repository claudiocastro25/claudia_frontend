...
import { uploadDocument, getUserDocuments } from '../../services/documentService';
...
const MessageInput = ({ 
  ...
  const handleUploadFile = async () => {
    if (!selectedFile || !processorAvailable || !conversationId) return;
  ...
  return (
    <Box>
      ...
      >
        ...
        <Tooltip 
          title={processorAvailable ? "Anexar documento" : "Processador de documentos indisponível"}
          placement="top"
        >
          <Box sx={{ display: 'inline-flex' }}>
            <IconButton 
              color="primary"
              component="label"
              disabled={!processorAvailable || disabled || !conversationId}
              sx={{
