...
const EnhancedProcessingFeedback = ({ status, progress, filename, error }) => {
  ...
  // CORREÇÃO: Se há progresso mas não há status definido, assumimos que está processando
  ...
  useEffect(() => {
    if (safeStatus === 'error' || safeStatus === 'completed') return;
  ...
  return (
    <Fade in={safeStatus === 'uploading' || safeStatus === 'processing' || safeProgress > 0}>
      <Paper 
